...
if (con == 5)
{
	...
	writer("event_brock_postbattle_0", -1, -1);
	con = 9;
}
if (con == 9 && exists(thiswriter) == 0)
{
	brock.hspeed = 4;
	if (brock.x >= (room_width + 20))
	{
		brock.hspeed = 0;
		alarm[2] = 120;
		aftercon = 0;
...
if (con == 28)
{
	writer("event_brock_postbattle_1", -1, -1);
...
if (con == 33)
{
	writer("event_brock_postbattle_2", -1, -1);
	con = 34;	
}
...
{
	brocksiner += 0.1;
	brock.vspeed = (sin(brocksiner) * 0.25);	
}