...
if (room == room_corridors_1) // corredores
{
	...
	if (global.flag[0] >= 0.5)
	{
		...
		if (exists(thatwriter) == 1 && global.flag[66] == 0.5)
		{
			music_paused[0] = 1;
			global.flag[66] = 0.75;
		}
		if (global.flag[66] == 0.75 && exists(thatwriter) == 0)
		{
			music_paused[0] = 0;
			global.flag[66] = 1;	
		}
...
{
	global.music[0] = -1;
	music_set(1, snd_ambient_wind, 0.75, 3, 1, 0.25, 1, 0.5);
}
if (room == room_corridors_2)
{
	global.music[0] = -1;
	if (global.flag[1] == 0.5)
		music_set(0, mus_m6, 1, 0, 0, 1, 1, 1);
}
if (room >= room_corridors_3)
{	
	music_set(0, mus_corridors, 1, 0.5, 1, 1, 1, 0.5);
	if (global.chara_murder >= 2)
	{
...
if (exists(obj_battle_quicker) == 0) || (_stop_music_on_quicker == 0)
{
	...
	if (room == room_corridors_12) // bc events
	{
		...
		if (global.flag[37] == 0.5)
		{
			global.music[MUSIC_NORMAL] = mus_bc_prebattle;
			global.music_gain[MUSIC_NORMAL] = 0;
		}
	}
