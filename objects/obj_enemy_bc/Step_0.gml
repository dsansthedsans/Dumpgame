...
if (active == 1)
{
	...
	else if (hurt_time >= 60)
	{
		if !(controller.enemy_spare[myself] >= 100 && dmg != 0)
		{
			...
			{	
				body.movement = 1;
				writer("battle_bubble_bc1", bubble_x, bubble_y);
				hurt_stage = 1;
			}
	...
	if (controller.enemy_type[myself] != 0)
	{
		...
		if (controller.battle_usedact == 1 && usedact == 0 && controller.enemy_target == myself)
		{
			...
			writer("battle_act_bc" + string(controller.level_heard), bubble_x, bubble_y);
			usedact = 1;
		}
		...
		if (controller.createbubble == 1 && createbubble == 1)
		{
			...
			if (body.movement == 2)
			{
				...
				{	
					bubbledelay = 0;
					global.writer_enemy = id;
					writer("battle_bubble_bc0", bubble_x, bubble_y);
					createbubble = 2;
					returnmain = 0;
				}
				else
